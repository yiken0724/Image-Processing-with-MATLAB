...
figure;
imshow(P3_new)
title('Image after double histogram equalization')
...
peak_x_improved = [8.930400; 249.0195];
peak_y_improved = [240.5469; 17.57920];
...
filter_image_with_window(dft_improved, peak_x_improved, peak_y_improved, 21);
...
imagesc(fftshift(spectrum_filtered.^0.1)); 
title('Filtered Power Spectrum of Interference Image (5 x 5 kernel)');
colormap('default');
...
figure;
imshow(filtered_diagonal_lines_image);
title('Filtered Interference Image (5 x 5 kernel)');
imwrite(filtered_diagonal_lines_image, fullfile('results', 'fft_primate_caged_5_5.png'));

dft_filtered = modify_dft(11, dft, peak_x, peak_y);

spectrum_filtered = abs(dft_filtered).^2; % create power spectrum of image
figure;
imagesc(fftshift(spectrum_filtered.^0.1)); 
title('Filtered Power Spectrum of Interference Image (11 x 11 kernel)');
colormap('default');
...
figure;
imshow(filtered_diagonal_lines_image);
title('Filtered Interference Image (11 x 11 kernel)');
imwrite(filtered_diagonal_lines_image, fullfile('results', 'fft_primate_caged_11_11.png'));
...
%% Perceptron first algorithm
function [w, iter, misclassifications] = perceptron_algo1(X, y, alpha, max_iter)
    w = zeros(size(X, 2), 1); % Initialization of weights
    misclassifications = zeros(max_iter,1); % Initialization of misclass
    for iter = 1:max_iter
        misclassified = 0;
        for k = 1:size(X, 1)
            xk = X(k, :)';
            if y(k) == 1 && (w' * xk) <= 0 % c1 sample misclassified
                w = w + alpha * xk;
                misclassified = misclassified + 1;
            elseif y(k) == -1 && (w' * xk) >= 0 % c2 sample misclassified
                w = w - alpha * xk;
                misclassified = misclassified + 1;
            end
        end
        misclassifications(iter) = misclassified; % store number of misclass for an iteration
        preds = sign(X * w); % Check for convergence
        if all(preds == y)
            fprintf('Converged at iteration %d\n', iter);
            misclassifications = misclassifications(1:iter);
            return;
        end
    end
    fprintf('Reached max iterations (%d)\n', max_iter); % Reached max iterations
    misclassifications = misclassifications(1:iter);
end

alpha = 1;
max_iter = 100;
% Lecture Slides Example (Simple)
X = [3 3 1; 1 1 1];   
y = [1; -1];   
[w, iter, misclassifications] = perceptron_algo1(X, y, alpha, max_iter);
figure;
plot(1:iter, misclassifications, '-o', 'LineWidth', 2);
xlabel('Iteration'); ylabel('Number of Misclassifications');
title('Convergence of Perceptron (Example 1)');
disp('Final weights (Example 1):');
disp(w);

% Other Example (More complex)
X = [2 3 1; 3 4 1; 4 5 1; 1 1 1; 1 2 1; 2 1 1]; 
y = [1; 1; 1; -1; -1; -1];  
[w, iter, misclassifications] = perceptron_algo1(X, y, alpha, max_iter);
figure;
plot(1:iter, misclassifications, '-o', 'LineWidth', 2);
xlabel('Iteration'); ylabel('Number of Misclassifications');
title('Convergence of Perceptron (Example 2)');
disp('Final weights (Example 2):');
disp(w);

% Other Example (Not linearly separable)
...
plot(1:iter, misclassifications, '-o', 'LineWidth', 2);
xlabel('Iteration'); ylabel('Number of Misclassifications');
title('Convergence of Perceptron (Example 3)');
disp('Final weights (Example 3):');
disp(w);

%% Perceptron second algorithm
function [w2, iter2, errors] = perceptron_algo2(X, y, alpha2, max_iter2)
    w2 = zeros(size(X, 2), 1);
    errors = zeros(max_iter2, 1);
    for iter2 = 1:max_iter2
        ...
        for k = 1:size(X, 1)
            ...
            w2 = w2 + alpha2 * error * X(k, :)'; % Gradient update
        end
        errors(iter2) = total_error;
        if iter2 > 1 && total_error < 1e-6
            fprintf('Early stopping at iteration %d\n', iter2);
            break;
        end
    end
    errors = errors(1:iter2);
end
...
X = [3 3 1; 1 1 1];   
y = [1; -1];   
[w2, iter2, errors] = perceptron_algo2(X, y, alpha2, max_iter2);
figure;
plot(1:iter2, errors, '-o', 'LineWidth', 2);
